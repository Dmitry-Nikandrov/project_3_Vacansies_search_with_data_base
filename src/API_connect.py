...
path_vac = os.getenv('path_vac')
path_emp = os.getenv('path_emp')
...
class APIConnector:
    ...
    def __init__(self):
        ...
        self.path_vac = path_vac
        self.path_emp = path_emp

    def get_hh_vacancies(self, companies=org_id_list):
        """подключается с hh.ru и получает информацию о вакансиях по выбранным компаниям"""
        list_vacancies = []
        for i in companies:
            self.params_vac["employer_id"] = i
            response_vac = requests.get(self.path_vac, params=self.params_vac)
    ...
    def save_employers_in_file(self, filename="data_employers.json"):
        ...
        data_list = self.get_hh_employers(org_id_list)
        path = os.path.abspath("./data")
        with open(os.path.join(path, filename), "w", encoding="utf-8") as file:
            file.write(json.dumps(data_list, ensure_ascii=False, indent=4))
        return f"Создан файл {filename}"
